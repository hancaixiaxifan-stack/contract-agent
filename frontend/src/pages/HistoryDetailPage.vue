...
onMounted(async () => {
  ...
  if (storedResult) {
    ...
    fromHome.value = route.query.from === "home"
...
const goBack = () => {
  router.push(fromHome.value ? "/" : "/history")
}
...
.summary-text {
  flex: 1; font-size: 14px; line-height: 1.6; color: var(--ink-soft); min-width: 200px;
}
...
.risks-count {
  margin-left: auto; font-size: 12px; color: var(--ink-muted); font-weight: 400;
}

.risks-body { flex: 1; overflow-y: auto; padding: 12px; }
...
.risk-card:hover { border-color: var(--ink-soft); box-shadow: var(--shadow-xs); }
...
.risk-point { font-size: 13px; font-weight: 600; color: var(--ink); flex: 1; }

.risk-location { font-size: 11px; color: var(--ink-muted); margin-bottom: 8px; }

.risk-quote {
  margin: 0; font-size: 12px; color: var(--ink-soft); font-style: italic;
  background: var(--parchment); padding: 8px 10px;
  border-radius: var(--r-sm); border-left: 2px solid var(--gold-border);
...
.no-risk-icon {
  ...
  font-size: 22px; color: var(--risk-low); margin-bottom: 14px;
}

.no-risk-text { font-size: 14px; color: var(--ink-muted); }

@media (max-width: 900px) {
  .main-split { flex-direction: column; height: auto; }
  .risks-panel { width: 100%; height: 400px; }
}
</style>
